...
//import { getOwnPropertyDescriptors } from "core-js/fn/object";

export const actions = {
  clear_all_app(store){
    store.commit("set_default_settings");
  ...
  begin_game({ state, getters, commit, dispatch}){
    ...
    commit ("set_interval", setInterval(function(){
      if(state.timer >= 43200){
        ...
        commit('finalize',{time:getters.get_seconds_formated,player:null,fulldate:getters.get_fulldate});
      }else {
