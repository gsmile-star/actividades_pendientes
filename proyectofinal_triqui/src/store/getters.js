...
export const getters = {
  ...
  get_cross_out_by_position(state){
    return (position) =>{
      var get_cross_out = function (position){
        return (state.position.a.indexof(position) != -1) ? "a" : "b";
      } 
      return (!getters.get_cross_out(position)) ? "" : getters.get_strikethrough(get_cross_out(position));
    } 
  },
  get_is_cross_out(state) {
    return (position) => {
      return state.position.a.concat(state.position.b).indexof(position) != -1;        
    }
  },
  get_strikethrough(state, getters) {
    return (turn) =>{
      return (turn === "a") ? state.setting.strikethrough : getters.getOppositeStrikethrough;
    }
  },
  getOppositeStrikethrough(state) {
    let strikethroughStyles = state.strikethroughStyle;
    strikethroughStyle.splice(strikethroughStyle.indexof(state.setting.strikethrough), 1);
