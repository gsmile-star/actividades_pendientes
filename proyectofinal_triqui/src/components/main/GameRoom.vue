  ...
  <section class="body">
    ...
    <popup v-if="!is_started">
      ...
      <ul class="ttt-prev-start">
        ...
        <game-options label-begin="start"></game-options>
      </ul>
       </template>
    </popup>
    <popup v-if="winner">
      ...
      <li v-if="winner.player" class="d-flex">
         <div class="d-flex vh-flex-center">WINNER:</div>
         <div>{{winner.player | strtoupper}}</div>
      </li>
      <li v-else class="d-flex">
        <div class="vh-flex-center w100">¡ DRAW !</div>
      </li>
      <li class="d-flex">
        <div class="d-flex vh-flex-center">TIME</div>
        <div>{{winner.TIME}}</div>
      </li>
       <game-options label-begin="restart"></game-options>
      </ul>
       </template>
    </popup>
  </section>
...
export default {
  ...
  methods:{
    ...
    getPosition:function(row,colum){
      return (((row+row)-(this.getDimensions()-row))+colum)-1;
    }
  },
  computed:{
    board_styles(){
